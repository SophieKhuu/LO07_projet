...
class ModelCompte {
   ...
   public static function getMyPatrimoine($nom, $prenom){
              try{
           $database = Model::getInstance();
           $query = "SELECT * , SUM(Valeur) over(ORDER BY Valeur) AS Capital 
                        from (
                                SELECT 'compte' as Catégorie, C.label, C.montant as Valeur 
                                        ...
                                        WHERE C.personne_id= P.id AND P.nom = :nom AND P.prenom = :prenom 
                            UNION 
                                SELECT 'résidence' as Catégorie, R.label, R.prix as Valeur 
                                        FROM residence as R, personne as P 
                                        WHERE R.personne_id=P.id AND P.nom = :nom AND P.prenom = :prenom
                            ) 
                        as patrimoine ";
           $statement= $database->prepare($query);
           $statement->execute([
